...
export default function RestaurantAuthPage() {
  ...
  const inputClass = "w-full bg-white border border-ink-200 rounded-xl px-3.5 py-2.5 pl-9 text-ink-900 placeholder-ink-400 text-sm focus:outline-none focus:ring-2 focus:ring-flame-500/20 focus:border-flame-500 transition"

  return (
    <div className="min-h-screen bg-ink-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <Link to="/" className="btn btn-ghost btn-sm mb-6 inline-flex gap-2 text-ink-500">
          <ArrowLeft size={15} /> Back to student app
        </Link>

        <div className="text-center mb-6">
          <div className="text-5xl mb-3">🏪</div>
          <h1 className="text-2xl font-black text-ink-900">Restaurant Portal</h1>
          <p className="text-ink-400 text-sm mt-1">Manage your cafeteria restaurant</p>
        </div>

        {/* Tabs */}
        <div className="flex bg-ink-100 rounded-2xl p-1 mb-5 border border-ink-200">
          {[['login', 'Owner Login', LogIn], ['register', 'Register', UserPlus]].map(([t, label, Icon]) => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-all ${tab === t ? 'bg-white text-ink-900 shadow-sm' : 'text-ink-500 hover:text-ink-700'}`}>
              <Icon size={13} />{label}
            </button>
          ))}
        </div>

        <div className="bg-white rounded-2xl p-6 border border-ink-200 shadow-sm">
          ...
          {tab === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="email" value={loginForm.email} onChange={e => setLoginForm(p => ({ ...p, email: e.target.value }))}
                    className={inputClass} placeholder="you@restaurant.com" required />
                </div>
              </div>
              <div>
              ...
              <div className="border-t border-ink-100 pt-3 text-xs text-ink-400 text-center">
                ...
                <button type="button" onClick={() => setTab('register')} className="text-flame-500 hover:text-flame-600 font-semibold">Register here</button>
              </div>
            </form>
          )}
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="bg-flame-50 border border-flame-200 rounded-xl p-3 text-xs text-flame-700">
              ...
              {/* Logo upload */}
              <div>
                <label className="block text-xs font-semibold text-ink-500 uppercase tracking-wider mb-1.5">Restaurant Logo</label>
                <div className="flex items-center gap-4">
                  <div
                    onClick={() => logoInputRef.current?.click()}
                    className="w-20 h-20 rounded-2xl border-2 border-dashed border-ink-200 hover:border-flame-500 bg-ink-50 flex items-center justify-center cursor-pointer transition overflow-hidden shrink-0">
                    {logoPreview
                  ...
                  <div className="text-xs text-ink-400">
                    ...
                    <p className="mt-0.5 text-ink-300">PNG, JPG up to 5 MB</p>
                    {logoPreview && (
                      <button type="button" onClick={() => { setLogoFile(null); setLogoPreview('') }}
                        className="mt-1.5 text-red-400 hover:text-red-600">Remove</button>
                    )}
                  </div>
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-ink-500 uppercase tracking-wider mb-1.5">Short Description</label>
                <textarea value={regForm.description} onChange={rf('description')} rows={3}
                  className="w-full bg-white border border-ink-200 rounded-xl px-3.5 py-2.5 text-ink-900 placeholder-ink-400 text-sm focus:outline-none focus:ring-2 focus:ring-flame-500/20 focus:border-flame-500 transition resize-none"
